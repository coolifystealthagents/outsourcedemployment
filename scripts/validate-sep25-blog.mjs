...
import {september25BlogBatch as posts} from '../app/blog/sep25-batch.ts';

const publicationDate='2026-09-25';
const words=value=>value.trim().split(/\s+/).filter(Boolean).length;
const inventory=fs.readdirSync(path.join(process.cwd(),'app/blog')).filter(name=>name.endsWith('.ts')&&name!=='sep25-batch.ts').map(name=>fs.readFileSync(path.join(process.cwd(),'app/blog',name),'utf8')).join('\n');
