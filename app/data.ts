...
import { august21BlogBatch } from './blog/aug21-batch';
import { august23BlogBatch } from './blog/aug23-batch';
...
import { september4BlogBatch } from './blog/sep4-batch';
import { september7BlogBatch } from './blog/sep7-batch';
...
import { september24BlogBatch } from './blog/sep24-batch';
...
import { august19ResearchBatch } from './research/aug19-batch';
import { august20ResearchBatch } from './research/aug20-research-batch';
import { august21ResearchBatch } from './research/aug21-research-batch';
import { august23ResearchBatch } from './research/aug23-research-batch';
import { august31ResearchBatch } from './research/aug31-research-batch';
import { september1ResearchBatch } from './research/sep1-research-batch';
import { september2ResearchBatch } from './research/sep2-research-batch';
import { september3ResearchBatch } from './research/sep3-research-batch';
import { september4ResearchBatch } from './research/sep4-research-batch';
import { september7ResearchBatch } from './research/sep7-research-batch';
...
type ResearchPost = { slug: string; title: string; excerpt: string; body: string[]; publishedDate?: string; modifiedDate?: string; openGraphModifiedDate?: string; handoff?: { href: string; label: string; body: string } };
...
const researchBodyV3 = (question: string, evidence: string, role: string, test: string, boundary: string, sources: string[]) => [
  `Research question: ${question}`,
  `Evidence frame: ${evidence} This is planning context, not proof of an individual worker's ability or a conclusion about a particular employment arrangement.`,
  `Role analysis: ${role} The distinction is between preparing a verified record and making the decision that changes a person's pay, status, access, or obligation.`,
  `Suggested test: ${test} The reviewer should score whether the record is complete, traceable, and stopped at the right uncertainty.`,
  'Management implication: begin with examples that have a known answer and a clear owner. Expand the lane only when the reviewer can see the same error pattern across several samples.',
  `Boundary: ${boundary} The owner retains decisions about money, employment status, policy, legal position, confidential disclosure, and access scope.`,
  'Evidence standard: record the source, date checked, person who reviewed the item, and the reason for every exception. A clean handoff is more useful than a confident guess.',
  'Sources:', ...sources,
  'Conclusion: a narrow role with visible evidence gives a Philippines staffing plan a fairer test than a broad title or an unmeasured promise.',
];
...
const researchPostsRaw: ResearchPost[] = [
  ...
  { slug: 'philippines-hr-document-administration-research', title: 'Philippines HR Document Administration: A Traceable Preparation Workflow', excerpt: 'Design document preparation around templates, completeness checks, and review trails rather than unsupervised HR decisions.', publishedDate: '2026-08-12', modifiedDate: '2026-09-24', openGraphModifiedDate: '2026-09-24', body: researchBody('HR document administration', '59.3% of employment in the Philippines was in services in the World Bank 2024 series', 'template population, field checks, version tracking, signature-status follow-up, and exception packets', 'the HR owner approves terms, legal language, signatures, employment status, policy exceptions, and release of final documents'), handoff: { href: '/services/employment-document-administration', label: 'Plan employment document administration', body: 'Use this service to organize approved templates, version checks, signature-status follow-up, and exception records. Your HR owner still approves terms, legal language, signatures, employment status, policy exceptions, and final document release.' } },
  ...
  { slug: 'philippines-payroll-input-reconciliation-research', title: 'Philippines Payroll Input Reconciliation: Evidence Before Approval', excerpt: 'Study how a support role can reconcile payroll inputs while finance retains approval over pay, exceptions, and release.', publishedDate: '2026-08-12', modifiedDate: '2026-09-15', openGraphModifiedDate: '2026-09-15', body: researchBodyV3('How can payroll input reconciliation improve accuracy without transferring pay authority?', 'The U.S. Department of Labor explains that wage and hour obligations depend on the facts of the employment relationship and applicable rules.', 'a coordinator can compare approved time, leave, deductions, and change records; they can flag mismatches and prepare a review packet.', 'use de-identified records containing one changed hour, one missing approval, one duplicate entry, and one unexplained variance.', 'the role may reconcile and route discrepancies, but must not approve pay, change a worker record, release funds, or interpret a legal exception.', [researchSources[7], researchSources[3], researchSources[4]]), handoff: { href: '/services/payroll-input-preparation', label: 'Plan payroll input preparation', body: 'Use a short payroll-input plan to name the records to compare, the review point, and the finance owner who approves pay, exceptions, record changes, and fund release.' } },
  { slug: 'philippines-benefits-enrollment-administration-research', title: 'Philippines Benefits Enrollment Administration: Records and Owner Decisions', excerpt: 'Separate enrollment record preparation from eligibility, plan, and employee decisions in a Philippines-based support role.', publishedDate: '2026-08-12', body: researchBodyV3('Which benefits administration tasks can be prepared from documented records while the employer keeps eligibility decisions?', 'The ILO provides public material on working conditions and labour standards, but a source cannot determine the rules for every employer, worker, or jurisdiction.', 'a coordinator can track received forms, identify missing fields, route questions, and maintain a dated status record against the approved plan.', 'test a small set of masked enrollment packets with missing signatures, conflicting dates, and a clear escalation case.', 'the role must not decide eligibility, disclose sensitive information, promise coverage, alter a plan, or give legal or tax advice.', [researchSources[3], researchSources[6], researchSources[7]]) },
  { slug: 'philippines-employee-records-retention-research', title: 'Philippines Employee Records Retention: A Source-and-Access Review', excerpt: 'Examine records inventory and access review as delegated work while retention and disclosure remain with the responsible owner.', publishedDate: '2026-08-12', modifiedDate: '2026-09-03', openGraphModifiedDate: '2026-09-03', body: researchBodyV3('What evidence should a records coordinator prepare before an owner makes a retention or disclosure decision?', 'The FTC recommends that businesses protect personal information through practical privacy and security safeguards appropriate to the data they hold.', 'a coordinator can inventory record classes, compare permission lists with approved access, flag stale copies, and prepare an exception register.', 'give the reviewer masked examples with duplicate files, an expired permission, and a record whose retention status is unclear.', 'the role must not delete records, approve retention exceptions, disclose employee information, or broaden access to resolve an inventory gap.', [researchSources[6], researchSources[4], researchSources[5]]), handoff: { href: '/services/employee-records-management', label: 'Plan employee records management', body: 'Use this service when you need a defined records lane for inventory checks, access reviews, and exception notes. Your responsible owner still decides record changes, disclosure, retention exceptions, employment status, and access.' } },
  { slug: 'philippines-leave-records-reconciliation-research', title: 'Philippines Leave Records Reconciliation: Separating Facts from Approval', excerpt: 'Use evidence-led reconciliation to prepare leave records without letting a support role decide entitlement or exceptions.', publishedDate: '2026-08-12', modifiedDate: '2026-09-12', openGraphModifiedDate: '2026-09-12', body: researchBodyV3('How can leave-record reconciliation surface mismatches without turning a coordinator into the approver?', 'The ILO working-conditions resource is a useful starting point for labour standards questions, but it does not replace advice tied to the employer and location.', 'a coordinator can compare approved requests, recorded balances, dates, and supporting notes, then route an exception with its source records.', 'test cases with one overlapping request, one missing approval, one corrected date, and one record that should be escalated.', 'the role cannot approve leave, interpret entitlement, change balances without authorization, or decide how a dispute should be resolved.', [researchSources[3], researchSources[7]]), handoff: { href: '/services/leave-tracking-support', label: 'Plan leave tracking support', body: 'Use this service when you need a clear lane for comparing leave requests, recorded balances, and missing approvals. Your authorized owner still decides entitlement, exceptions, balances, and dispute outcomes.' } },
  { slug: 'philippines-onboarding-records-coordination-research', title: 'Philippines Onboarding Records Coordination: Completeness Without Employment Decisions', excerpt: 'Research the evidence needed for an onboarding coordinator to prepare a clean handoff while HR retains decision rights.', publishedDate: '2026-08-12', modifiedDate: '2026-09-06', openGraphModifiedDate: '2026-09-06', body: researchBodyV3('What can onboarding records coordination prove before an HR owner confirms a new hire is ready?', 'The ILO publishes guidance on working conditions and labour standards, which helps frame questions but does not certify a specific hiring arrangement.', 'a coordinator can track approved documents, account requests, orientation attendance, and open questions against a named owner and start-date plan.', 'use sanitized packets with a missing document, conflicting identity field, delayed account request, and an ordinary complete case.', 'the role must not change employment terms, decide eligibility, answer policy questions beyond approved wording, or grant access without authorization.', [researchSources[3], researchSources[4], researchSources[5]]), handoff: { href: '/services/new-hire-onboarding', label: 'Plan new-hire onboarding', body: 'Use this service when you need a clear lane for approved documents, account-request follow-up, orientation records, and open questions. Your HR owner still decides eligibility, employment terms, policy answers, access, and start approval.' } },
  { slug: 'philippines-candidate-scheduling-evidence-research', title: 'Philippines Candidate Scheduling Evidence: Coordination Before Selection', excerpt: 'Assess scheduling and application-completeness work without outsourcing hiring judgment or candidate decisions.', publishedDate: '2026-08-12', modifiedDate: '2026-08-26', body: researchBodyV3('Which recruiting coordination outputs help a hiring owner decide without hiding gaps in the candidate record?', 'The IRS explains that worker classification depends on the actual relationship, including control and independence, rather than a label alone.', 'a coordinator can schedule approved interview steps, check required fields, preserve structured notes, and flag missing evidence.', 'test a sanitized queue with a timezone conflict, incomplete application, duplicate candidate, and a clear reschedule request.', 'the role must not rank candidates, reject applicants, set compensation, interpret qualifications, or make a hiring promise.', [researchSources[8], researchSources[3]]), handoff: { href: '/services/candidate-sourcing-coordination', label: 'Plan candidate sourcing coordination', body: 'Use this service when you need a clear lane for interview scheduling, record checks, and follow-up. Your hiring owner still chooses candidates, decides qualifications, and makes every offer.' } },
  { slug: 'philippines-workforce-reporting-definitions-research', title: 'Philippines Workforce Reporting: Definitions Before Dashboard Updates', excerpt: 'Show how a reporting coordinator can protect metric definitions and source traceability while managers retain interpretation.', publishedDate: '2026-08-12', body: researchBodyV3('What should a workforce reporting coordinator verify before a manager relies on a headcount or attendance number?', 'World Bank indicators provide country-level context, but they do not define an individual company metric or validate a private workforce dataset.', 'a coordinator can refresh approved inputs, check dates and units, compare totals, document exclusions, and route discrepancies with source links.', 'use a small report with one stale date, one changed denominator, one missing field, and one correctly reproducible result.', 'the role must not redefine metrics, publish sensitive workforce information, alter source records, or turn a discrepancy into a management conclusion.', [researchSources[0], researchSources[1], researchSources[2], researchSources[9]]) },
  { slug: 'philippines-remote-access-removal-research', title: 'Philippines Remote Access Removal: Evidence for a Clean Role Exit', excerpt: 'Analyze access-removal evidence for Philippines-based staff while owners retain authority over systems, data, and exceptions.', publishedDate: '2026-08-12', body: researchBodyV3('What should an access coordinator prove when a remote role changes or ends?', 'NIST Cybersecurity Framework 2.0 and CISA password guidance provide general control language for identifying, protecting, and reviewing access.', 'a coordinator can compare the approved access list with system records, collect confirmation from system owners, and maintain an exception log.', 'test a role with a shared mailbox, a named application account, a pending transfer, and an account that should be disabled.', 'the role must not create privileged access, export data, decide whether an exception is safe, or treat a missing confirmation as proof of removal.', [researchSources[4], researchSources[5], researchSources[6]]) },
  { slug: 'philippines-customer-support-escalation-research', title: 'Philippines Customer Support Escalation: Evidence Before a Promise', excerpt: 'Research how ticket preparation and escalation records can support customer service without transferring refunds or policy authority.', publishedDate: '2026-08-12', body: researchBodyV3('How can a customer support coordinator prepare a useful escalation without promising an outcome?', 'The FTC privacy and security guidance is relevant when support records contain personal information, while service decisions still depend on the company policy and facts.', 'a coordinator can classify the issue, gather the approved account history, draft a response for review, and identify the decision owner.', 'test a sanitized queue with a routine request, a duplicate case, a data-access request, and a request that needs a policy owner.', 'the role must not issue refunds, make legal statements, change account permissions, disclose private data, or promise a remedy outside approved language.', [researchSources[6], researchSources[4]]) },
  { slug: 'philippines-staffing-provider-evidence-research', title: 'Philippines Staffing Provider Evidence: Questions Behind the Service Model', excerpt: 'A research note on comparing employment, management, records, and security evidence before choosing a Philippines staffing model.', publishedDate: '2026-08-12', body: researchBodyV3('What evidence should a buyer request before relying on a Philippines staffing provider for a defined role?', 'The IRS distinction between employee and independent contractor status shows why labels alone do not answer who controls the work or carries the relevant obligations.', 'a buyer can compare the named contracting parties, daily manager, records supplied, screening evidence, access controls, and exit responsibilities.', 'score two anonymized provider descriptions against the same role brief and record which claims have documents, owners, dates, and unresolved gaps.', 'the research note does not select a provider or give legal advice. The buyer retains contract, classification, security, and employment decisions.', [researchSources[8], researchSources[4], researchSources[5], researchSources[3]]) },
];
