...
export default async function Post({params}:{params:Promise<{slug:string}>}){
  ...
  if(!article){
    const customBody = Array.isArray(post.body) ? post.body : [];
    return <><Header/><main className="section"><JsonLd data={{'@context':'https://schema.org','@type':'Article',headline:post.title,description:post.excerpt,url,image:post.image,datePublished:publishedDate,citation:Array.isArray(post.sources)?post.sources.map((source:{url:string})=>source.url):[]}}/><article className="container guide-article"><p className="eyebrow">Philippines staffing blog · <time dateTime={publishedDate}>Published {new Date(`${publishedDate}T00:00:00Z`).toLocaleDateString('en-US',{month:'long',day:'numeric',year:'numeric',timeZone:'UTC'})}</time></p><h1>{post.title}</h1><p className="lead">{post.excerpt}</p>{post.image&&<img src={post.image} alt={`Illustration for ${post.title}`} width="1200" height="675" style={{width:'100%',height:'auto',borderRadius:'18px'}}/>}<div className="card">{customBody.length ? customBody.map((paragraph:string,index:number)=><p key={index}>{paragraph}</p>) : <><h2>Define the work before the hire</h2><p>Write the recurring tasks, examples, tools, schedule, and approval boundaries before a Filipino specialist begins. A clear role lets you compare a staffing provider, an employer-of-record arrangement, and direct management on the work that actually needs doing.</p><h2>Build a controlled handoff</h2><p>Begin with low-risk samples and only the permissions required for the approved Philippines-based workload. Use named accounts and multifactor authentication. The <a href="https://www.nist.gov/cyberframework" target="_blank" rel="noreferrer">NIST Cybersecurity Framework</a> and <a href="https://www.cisa.gov/secure-our-world/use-strong-passwords" target="_blank" rel="noreferrer">CISA password guidance</a> are useful control references; they do not replace advice for your specific situation.</p><h2>Review outputs and keep decisions visible</h2><p>Use a weekly check of completed work, open decisions, and changing priorities. The specialist can prepare records, drafts, and reports; your named manager should retain approvals, exceptions, spending, policy calls, and sensitive employee decisions.</p></>}</div>{Array.isArray(post.sources)&&<section className="source-list" aria-labelledby="guide-sources-title"><h2 id="guide-sources-title">Sources</h2><ul>{post.sources.map((source:{name:string;url:string})=><li key={source.url}><a href={source.url} target="_blank" rel="noreferrer">{source.name}</a></li>)}</ul></section>}{typeof post.cta==='string'&&<p className="related-card"><a href={post.cta}>Explore the related Outsourced Employment service</a></p>}<p className="article-date">This guide is general information, not legal, tax, or employment advice.</p></article><CTA/></main><Footer/></>;
  }
