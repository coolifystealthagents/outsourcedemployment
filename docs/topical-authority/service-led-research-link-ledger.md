# Service-led research link ledger

This planning ledger connects existing Philippines-only employment research to existing service pillars. It does not add claims, create routes, or change reader-facing output. A future route edit must use the exact source and destination listed here, retain the named owner boundary, refresh the post-specific modified date, and prove the link inside the rendered article.

## Confirmed service pillars

- `/services/candidate-sourcing-coordination` — candidate sourcing coordination
- `/services/employee-records-management` — employee records management
- `/services/employment-document-administration` — employment document administration
- `/services/new-hire-onboarding` — new-hire onboarding
- `/services/leave-tracking-support` — leave tracking support
- `/services/payroll-input-preparation` — payroll input preparation

## Research-to-service queue

| Status | Existing research URL | Existing service URL | Reader question | Boundary to retain | Next action |
| --- | --- | --- | --- | --- | --- |
| Delivered | `/research/philippines-candidate-scheduling-evidence-research` | `/services/candidate-sourcing-coordination` | How can a manager organize interview scheduling and record checks without handing over selection? | The hiring owner chooses candidates, sets qualifications, and makes offers. | Do not add a second handoff. Recheck the rendered article before any related edit. |
| Delivered | `/research/philippines-employee-records-retention-research` | `/services/employee-records-management` | What can a records coordinator prepare before the responsible owner decides retention or disclosure? | The owner decides record changes, disclosure, retention exceptions, employment status, and access. | Delivered locally by `13a4328e7ac40baacaf02bc22b8b9ab4be651412`; the built article has one route-local handoff. Do not add a second link. |
| Delivered locally; deployment pending public verification | `/research/philippines-hr-document-administration-research` | `/services/employment-document-administration` | Which document checks can be prepared before an HR owner approves the terms, signatures, or release? | The HR owner decides terms, legal language, signatures, employment status, policy exceptions, and final release. | Rendered source: `29de891568368a79874b861f29d3a4d4cf395415` passed local artifact proof with one route-local handoff, canonical, Article/OG modified date `2026-09-24`, and sitemap loc. The separate status record is `7eb04802d3e59d7d87afac179f5a98a1d1a6c1bd`; no approved deployment target is recorded, so do not add a duplicate handoff. |
| Delivered | `/research/philippines-onboarding-records-coordination-research` | `/services/new-hire-onboarding` | Which onboarding records should be complete before an HR owner confirms readiness? | The HR owner decides terms, employment status, policy answers, and access. | Delivered locally by `7bf10865098d9a32a84a55604f0a4dd075c92d41`; the built article has one route-local handoff. Do not add a second link. |
| Publicly verified | `/research/philippines-leave-records-reconciliation-research` | `/services/leave-tracking-support` | How can a coordinator surface leave-record conflicts without deciding entitlement? | The owner decides entitlement, exceptions, balances, and dispute outcomes. | Rendered source: `b32bf53eeb20c7411c3c468b92f4513fddad1da6` has one route-local marker and service href, canonical, Article/OG modified date `2026-09-12`, and sitemap loc. Later apex and www checks verified the same marker and date; do not add a duplicate handoff. |
| Delivered locally; deployment pending public verification | `/research/philippines-payroll-input-reconciliation-research` | `/services/payroll-input-preparation` | What evidence should finance review before payroll input is approved? | Finance retains pay approval, exceptions, employee-record changes, and fund release. | Rendered source: `561672993c42eb291a14b3619dec7ca399c858bd` passed local proof: canonical source H1, one route-local marker and service href, Article/OG modified date `2026-09-15`, and source/destination sitemap locations (this sitemap has no `lastmod`). The repository has no approved Coolify application identifier or deployment manifest. Cache-busted apex and www returned 200 HTML with the canonical H1 but lacked the marker and modified date; preserve rendered-source commit `561672993c42eb291a14b3619dec7ca399c858bd` and do not add a duplicate handoff. |

## Execution rules

1. Work one row at a time and never add a link where the same service already appears inside the source article.
2. Keep the service destination Philippines-specific and use the route record rather than a renderer exception when the model supports a handoff.
3. Verify the source and destination H1, canonical URL, generated sitemap entries, and the source article's route-local link count after each public-facing change.
4. Treat this ledger as source-only planning. It is not a deployment or public-release record.
